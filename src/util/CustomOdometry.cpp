...
/* -------------------- Sensor References -------------------- */
pros::Rotation & CustomOdometry::meF = def::track_encoder_forward; // left tracking wheel encoder
pros::Rotation & CustomOdometry::meS = def::track_encoder_side; // right tracking wheel encoder
...
{
    return {(double)meF.get_position()/100.5, (double)meS.get_position()/100.5,
            ((isinf(mimu1.get_rotation()*1.024) ? 0 : mimu1.get_rotation()*1.024) +
             (isinf(mimu2.get_rotation()*1.024) ? 0 : mimu2.get_rotation()*1.024)) *
                M_PI / 180 / 2};

                // return {(double)meF.get_position(), (double)meS.get_position(),
